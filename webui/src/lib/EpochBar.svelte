<script>
 ...
 let data = $derived(epochDataInfo.data)
 let progress = $derived((parseInt(data.slot) * 100 / 432000).toFixed(2))
</script>
...
<div class="has-text-centered">
  <p>
    ...
    <span class="is-inline-block">
      epoch {parseInt(data.epoch)} ( {parseInt(data.slot)} / 432000 ) {progress} % ( ~ {data.epoch_remaining_duration} ) [ tip: {data.koios_tip_block_height} ]
    </span>
